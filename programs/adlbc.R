...
debugonce(combine_supp)
debugonce(combine_supp_by_idvar)
lb1 <- metatools::combine_supp(lb, supplb) %>% as_tibble()
lb1 <- derive_vars_suppqual(lb, supplb, domain = 'LB')

lb1 <- create_var_from_codelist(lb, adlbc_spec, LBTESTCD, Term, decode_to_code = F)

adlbc_spec$codelist %>% as_tibble() %>% unnest(everything()) %>% filter(code_id=='PARAMCD_ADLBC')
...
adlbc1 <- lb %>% filter(LBCAT=='CHEMISTRY') %>%
